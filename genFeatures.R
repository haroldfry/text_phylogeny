...
genFeatures <- function(sample1, sample2, document1, document2, corpus, label, lang="en", stemDict) {   
   ...
   if (lang == "la") {
      ...
      #pastte individual elements together
   ...
   # 5 MATCH_STEM_2: number of stem-based matches in sample2
   if (lang == "la") {
   ...
   # 6 MATCH_STEM_BOTH: number of stem-based matches in both
   MATCH_STEM_BOTH <- MATCH_STEM_1 + MATCH_STEM_2
   vecStr <- paste(vecStr, " ", index, ":", MATCH_STEM_BOTH, sep="")
   index <- index + 1

   # 7 MATCH_STEM_UNIQ: number of unique forms among words matching by stem
   if (lang == "la") {
   ...
   # 8 MATCH_WORD_UNIQ: number of unique forms among words matching by word
   ...
   index <- index + 1
    ...
    # 24 F_M_COR_INV_1: Word matches corpus-wide sum of (1/F) for each freq. F in sample 1
    F_M_COR_INV_1 <- f_inv(matchList1, F_M_COR_AVG_1$matchListFreqs)
    vecStr <- paste(vecStr, " ", index, ":", F_M_COR_INV_1, sep="")
    index <- index + 1

    # 25 F_M_COR_INV_2: Word matches corpus-wide sum of (1/F) for each freq. F in sample 2
    F_M_COR_INV_2 <- f_inv(matchList2, F_M_COR_AVG_2$matchListFreqs)
    vecStr <- paste(vecStr, " ", index, ":", F_M_COR_INV_2, sep="")
    index <- index + 1

    # 26 F_M_COR_INV_BOTH: Word matches corpus-wide inv. freq. both
   ...
   # 27 F_P_DOC_AVG_1: All words doc. specific mean freq. in sample 1
   ...
   # 28 F_P_DOC_AVG_2: All words doc. specific mean freq. in sample 2
   ...
   # 29 F_P_DOC_AVG_BOTH: All words doc. specific mean freq. both
  ...
  # vecStr <- paste(vecStr, " ", index, ":", TF_M_T_AVG_BOTH, sep="")
   ...
   # 48 TF_M_T_SUM_1: Sum of TF-IDF scores of matching words for sample 1
   #TF_M_T_SUM_1 <- TF_M_T_AVG_1$runningTfIdf
   #vecStr <- paste(vecStr, " ", index, ":", TF_M_T_SUM_1, sep="")
   #index <- index + 1

   # 49 TF_M_T_SUM_2: Sum of TF-IDF scores of matching words for sample 2
   #TF_M_T_SUM_2 <- TF_M_T_AVG_2$runningTfIdf
   #vecStr <- paste(vecStr, " ", index, ":", TF_M_T_SUM_2, sep="")
   #index <- index + 1

   # 50 TF_M_T_SUM_BOTH: Sum of TF-IDF scores of matching words for both
   #TF_M_T_SUM_BOTH <- TF_M_T_SUM_1 + TF_M_T_SUM_2
   #vecStr <- paste(vecStr, " ", index, ":", TF_M_T_SUM_BOTH, sep="")
   #index <- index + 1

   # 51 TF_M_T_MAX_1: Max of TF-IDF scores of matching words for sample 1
   #TF_M_T_MAX_1 <- TF_M_T_AVG_1$maxTfIdf
   #vecStr <- paste(vecStr, " ", index, ":", TF_M_T_MAX_1, sep="")
   #index <- index + 1

   # 52 TF_M_T_MAX_2: Max of TF-IDF scores of matching words for sample 2
   #TF_M_T_MAX_2 <- TF_M_T_AVG_2$maxTfIdf
   #vecStr <- paste(vecStr, " ", index, ":", TF_M_T_MAX_2, sep="")
   #index <- index + 1

   # 53 TF_M_T_MAX_BOTH: Max of TF-IDF scores of matching words for both
...
#   index <- index + 1
   ...
   # 63 SPAN_1: The distance between the furthest two matching words in sample 1
   SPAN_1 <- span(matchListFull1, sample1Stemmed)
   vecStr <- paste(vecStr, " ", index, ":", SPAN_1$ftr, sep="")
   index <- index + 1

   # 64 SPAN_2: The distance between the furthest two matching words in sample 2
   SPAN_2 <- span(matchListFull2, sample2Stemmed)
   vecStr <- paste(vecStr, " ", index, ":", SPAN_2$ftr, sep="")
   index <- index + 1

   # 65 SPAN_BOTH: The combined distance between the furthest two matching words for both
   SPAN_BOTH <- SPAN_1$ftr + SPAN_2$ftr
   vecStr <- paste(vecStr, " ", index, ":", SPAN_BOTH, sep="")
   index <- index + 1

   # 66 D_F_DOC_1: is the distance between the lowest-freq words using doc-specific freqs for sample 1
   D_F_DOC_1 <- d_f(matchList1, document1, SPAN_1$allWordsRep)
   vecStr <- paste(vecStr, " ", index, ":", D_F_DOC_1, sep="")
   index <- index + 1

   # 67 D_F_DOC_2: is the distance between the lowest-freq words using doc-specific freqs for sample 2
   D_F_DOC_2 <- d_f(matchList2, document2, SPAN_2$allWordsRep)
   vecStr <- paste(vecStr, " ", index, ":", D_F_DOC_2, sep="")
   index <- index + 1

   # 68 D_F_DOC_BOTH: is the distance between the lowest-freq words using doc-specific freqs for both samples
   D_F_DOC_BOTH <- D_F_DOC_1 + D_F_DOC_2
   vecStr <- paste(vecStr, " ", index, ":", D_F_DOC_BOTH, sep="")
   index <- index + 1

    # 69 D_F_COR_1: is the distance between the lowest-freq words using corpus-wide freqs for sample 1
    D_F_COR_1 <- d_f(matchList1, corpusStemmed, SPAN_1$allWordsRep)
    vecStr <- paste(vecStr, " ", index, ":", D_F_COR_1, sep="")
    index <- index + 1

    # 70 D_F_COR_2: is the distance between the lowest-freq words using corpus-wide freqs for sample 2
    D_F_COR_2 <- d_f(matchList2, corpusStemmed, SPAN_2$allWordsRep)
    vecStr <- paste(vecStr, " ", index, ":", D_F_COR_2, sep="")
    index <- index + 1

    # 71 D_F_COR_BOTH: is the distance between the lowest-freq words using corpus-wide freqs for both samples
    D_F_COR_BOTH <- D_F_COR_1 + D_F_COR_2
    vecStr <- paste(vecStr, " ", index, ":", D_F_COR_BOTH, sep="")
    index <- index + 1

   # 72 D_TF_T_1: is the distance between the two highest tfidf words for sample 1
#   D_TF_T_1 <- d_tf(matchList1, document1, SPAN_1$allWordsRep, corpusIndFiles)
#   vecStr <- paste(vecStr, " ", index, ":", D_TF_T_1, sep="")
#   index <- index + 1
#
#   # 73 D_TF_T_2: is the distance between the two highest tfidf words for sample 2
#   D_TF_T_2 <- d_tf(matchList2, document2, SPAN_2$allWordsRep, corpusIndFiles)
#   vecStr <- paste(vecStr, " ", index, ":", D_TF_T_2, sep="")
#   index <- index + 1
#
#   # 74 D_TF_T_BOTH: is the distance between the two highest tfidf words for both samples
#   D_TF_T_BOTH <- D_TF_T_1 + D_TF_T_2
#   vecStr <- paste(vecStr, " ", index, ":", D_TF_T_BOTH, sep="")
#   index <- index + 1

   # 75 LD: Levenshtein edit distance
   LD <- levenshteinDist(sample1, sample2)
   vecStr <- paste(vecStr, " ", index, ":", LD, sep="")
   index <- index + 1

   # 76 CHR_1_GR_CNT: fraction of matching character-level unigrams out of total
   ...
   # 77 CHR_2_GR_CNT: fraction of matching character-level bigrams out of total
   ...
   index <- index + 1
    ...
    # 78 CHR_3_GR_CNT: fraction of matching character-level trigrams out of total
   ...
   # 79 CHR_2_GR_FRQ: similarity of bigram frequencies between two strings
   ...
   # 80 CHR_3_GR_FRQ: similarity of trigram frequencies between two strings
